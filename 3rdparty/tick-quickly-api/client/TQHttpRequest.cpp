...
TQHttpRequestInput::TQHttpRequestInput() {
    initialize();
}

TQHttpRequestInput::TQHttpRequestInput(QString v_url_str, QString v_http_method) {
    initialize();
    url_str = v_url_str;
    http_method = v_http_method;
}

void TQHttpRequestInput::initialize() {
    var_layout = NOT_SET;
    url_str = "";
    http_method = "GET";
}

void TQHttpRequestInput::add_var(QString key, QString value) {
    vars[key] = value;
}

void TQHttpRequestInput::add_file(QString variable_name, QString local_filename, QString request_filename, QString mime_type) {
    TQHttpFileElement file;
...
TQHttpRequestWorker::TQHttpRequestWorker(QObject *parent, QNetworkAccessManager *_manager)
...
TQHttpRequestWorker::~TQHttpRequestWorker() {
...
QMap<QString, QString> TQHttpRequestWorker::getResponseHeaders() const {
    return headers;
}

TQHttpFileElement TQHttpRequestWorker::getHttpFileElement(const QString &fieldname) {
    if (!files.isEmpty()) {
    ...
    return TQHttpFileElement();
}

QByteArray *TQHttpRequestWorker::getMultiPartField(const QString &fieldname) {
...
void TQHttpRequestWorker::setTimeOut(int timeOutMs) {
...
void TQHttpRequestWorker::setWorkingDirectory(const QString &path) {
    if (!path.isEmpty()) {
        workingDirectory = path;
    }
}

void TQHttpRequestWorker::setResponseCompressionEnabled(bool enable) {
    isResponseCompressionEnabled = enable;
}

void TQHttpRequestWorker::setRequestCompressionEnabled(bool enable) {
    isRequestCompressionEnabled = enable;
}

int  TQHttpRequestWorker::getHttpResponseCode() const{
    return httpResponseCode;
}

QString TQHttpRequestWorker::http_attribute_encode(QString attribute_name, QString input) {
...
void TQHttpRequestWorker::execute(TQHttpRequestInput *input) {
    ...
    if (input->var_layout == ADDRESS || input->var_layout == URL_ENCODED) {
        ...
        // add files
        for (QList<TQHttpFileElement>::iterator file_info = input->files.begin(); file_info != input->files.end(); file_info++) {
    ...
    if (input->request_body.size() > 0) {
        ...
        if(!isFormData && (input->var_layout != MULTIPART) && isRequestCompressionEnabled){
            request_content.append(compress(input->request_body, 7, TQCompressionType::Gzip));
        } else {
    ...
    if (TQHttpRequestWorker::sslDefaultConfiguration != nullptr) {
        request.setSslConfiguration(*TQHttpRequestWorker::sslDefaultConfiguration);
    }
    ...
    if (reply != nullptr) {
        ...
        connect(reply, &QNetworkReply::downloadProgress, this, &TQHttpRequestWorker::downloadProgress);
...
void TQHttpRequestWorker::on_reply_finished(QNetworkReply *reply) {
    bool codeSts = false;
...
void TQHttpRequestWorker::on_reply_timeout(QNetworkReply *reply) {
...
void TQHttpRequestWorker::process_response(QNetworkReply *reply) {
...
QByteArray TQHttpRequestWorker::decompress(const QByteArray& data){
    
    Q_UNUSED(data);
    return QByteArray();
}

QByteArray TQHttpRequestWorker::compress(const QByteArray& input, int level, TQCompressionType compressType) {
...
QSslConfiguration *TQHttpRequestWorker::sslDefaultConfiguration;
